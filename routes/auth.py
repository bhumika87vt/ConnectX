...
def login():
    ...
    # ---- DEVELOPER SHORTCUT: FORCE ADMIN ROLE ----
    # If the logging-in user has the username 'admin', fix their role in MySQL right now
    if user.username == "admin" and user.role != "admin":
        user.role = "admin"
        db.session.commit()  # Permanently updates the row from 'user' to 'admin'
    # -----------------------------------------------
    ...
    session[SESSION_ROLE] = user.role  # This will now correctly load 'admin' into your session cookie
