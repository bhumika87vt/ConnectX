...
def _admin_guard():
    """Task 15: require admin via session role.
    Developer shortcut: Strictly intercepts and overwrites the DB role 
    before validating session data.
    """
    ...
    # ---- FORCE PYTHON DATABASE OVERRIDE ----
    # This runs immediately regardless of what cookie state your browser is in
    if user.username == "admin" and user.role != "admin":
        user.role = "admin"
        db.session.commit()  # Saves 'admin' to MySQL table right now!

    # Synchronize session state with the fresh database records
    if user.role == "admin":
